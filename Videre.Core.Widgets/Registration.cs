...
namespace Videre.Core.Widgets
{
    public class Registration : IWidgetRegistration
    {
        public int Register()
        {
            ...
            updates += CoreServices.Update.Register(new CoreModels.AttributeDefinition() { GroupName = "Authentication", Name = "PasswordExpiresDays", DefaultValue = "", DataType = "number", Required = false, LabelKey = "PasswordExpiresDays.Text", LabelText = "Password Expires (days)", TooltipKey = "PasswordExpiresDaysTooltip.Text", TooltipText = "Days until password expires (leave blank for no expiration)" });
