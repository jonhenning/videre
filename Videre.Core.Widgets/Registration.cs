...
namespace Videre.Core.Widgets
{
    public class Registration : IWidgetRegistration
    {
        public int Register()
        {
            ...
            updates += CoreServices.Update.Register(new CoreModels.AttributeDefinition() { GroupName = "Authentication", Name = "PasswordWarnDays", DefaultValue = null, DataType = "number", Required = false, LabelKey = "PasswordWarnDays.Text", LabelText = "Password Expires Warning (days)", TooltipKey = "PasswordExpiresWarnDaysTooltip.Text", TooltipText = "Days until password expires warning displays (leave blank for no expiration)" });
